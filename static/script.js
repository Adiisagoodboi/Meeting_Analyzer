...
function insertDemoSample() {
  ...
  const demoAnalysis = {
    ...
    summary: "This demo shows how the agenda, summary, key decisions, action items, and per-speaker relevance with pros, cons, and verdicts appear in the UI.",
    key_decisions: [
      "Proceed with the API integration this sprint.",
      "Add one more QA cycle before release."
    ],
    action_items: [
      { task: "Complete the API integration", owner: "Rohan", due: "Friday" },
      { task: "Raise a ticket for the deployment pipeline blocker", owner: "Shourya", due: "" },
      { task: "Set up the QA test matrix", owner: "Isha", due: "" }
    ],
    speakers: [
      {
        name: "Aditya",
...
function renderContributionAnalysis(analysis) {
  ...
  // Key decisions
  const decisionsList = document.getElementById('analysisDecisionsList');
  if (decisionsList) {
    const decisions = Array.isArray(analysis.key_decisions) ? analysis.key_decisions.filter(Boolean) : [];
    decisionsList.innerHTML = '';
    if (decisions.length === 0) {
      decisionsList.innerHTML = '<li class="muted small">No key decisions recorded.</li>';
    } else {
      decisions.forEach(d => {
  ...
  // Action items
  const actionsList = document.getElementById('analysisActionsList');
  if (actionsList) {
    const actions = Array.isArray(analysis.action_items) ? analysis.action_items : [];
    actionsList.innerHTML = '';
    if (actions.length === 0) {
      actionsList.innerHTML = '<div class="muted small">No action items identified.</div>';
    } else {
      actions.forEach(a => {
        const task = (a && (a.task ?? a.action ?? a.item)) || String(a || '');
        const owner = (a && (a.owner ?? a.assignee ?? a.who)) || 'unassigned';
        const due = (a && (a.due ?? a.deadline ?? a.when)) || '';
        const row = document.createElement('div');
        ...
        row.style.background = 'rgba(127,182,255,0.08)';
        row.style.borderLeft = '3px solid rgba(127,182,255,0.5)';
