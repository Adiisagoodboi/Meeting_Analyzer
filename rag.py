...
def _normalize_key_decisions(val: Any) -> List[str]:
    """Coerce GPT 'key_decisions' into a clean list of non-empty strings."""
    out: List[str] = []
    ...
    if isinstance(val, list):
        for item in val:
            if isinstance(item, dict):
                item = item.get("decision") or item.get("text") or ""
            s = str(item or "").strip()
            if s:
                out.append(s)
    return out
...
def _normalize_action_items(val: Any) -> List[Dict[str, str]]:
    """Coerce GPT 'action_items' into a list of {task, owner, due} dicts."""
    out: List[Dict[str, str]] = []
    if isinstance(val, list):
        for item in val:
            if isinstance(item, dict):
                task = str(item.get("task") or item.get("action") or item.get("item") or "").strip()
                owner = str(item.get("owner") or item.get("assignee") or item.get("who") or "unassigned").strip()
                due = str(item.get("due") or item.get("deadline") or item.get("when") or "").strip()
            else:
                task = str(item or "").strip()
                owner, due = "unassigned", ""
            if task:
                out.append({"task": task, "owner": owner or "unassigned", "due": due})
    return out
...
def sanitize_and_trust_gpt(parsed_obj: Any,
    ...
    key_decisions = _normalize_key_decisions(parsed.get("key_decisions"))
...
3) List the key decisions made in the meeting as short, self-contained bullet strings.
   If no clear decisions were made, return an empty list.
4) List concrete action items. For each, capture WHAT needs to be done, WHO owns it
   (use the speaker name if attributable, otherwise "unassigned"), and any DUE date/timeframe
   mentioned (otherwise ""). If none, return an empty list.
5) For each speaker, evaluate their contribution relative to the agenda and return:
...
6) Return ONLY a JSON object with fields:
{{
  "agenda": "string",
  "summary": "string",
...
def main():
    ...
    try:
        ...
        talk_times = compute_talk_times(entries)
        word_counts = compute_word_counts(entries)
        fallback = {"agenda": "", "summary": "", "key_decisions": [], "action_items": [], "speakers": []}
    ...
    final_obj = {
        ...
        "key_decisions": _normalize_key_decisions(result.get("key_decisions")),
